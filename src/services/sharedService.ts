...
export const filterUserBySearchService = async (pageNumber: number,searchValue) => {
    try {
        const response = await apiCall.get(`/admins/users?page=${pageNumber}&search=${searchValue}`, {
            headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
            },
...
export const filterUserByStateService = async (pageNumber: number,is_active:number) => {
    try {
        const response = await apiCall.get(`/admins/users?page=${pageNumber}&is_active=${is_active}`, {
            headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
            },
...
export const filterUserBySubscriptionService = async (pageNumber: number,is_subscribed:number) => {
    try {
        const response = await apiCall.get(`/admins/users?page=${pageNumber}&is_subscription=${is_subscribed}`, {
            headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
            },
