...
export default function DataTable({
    ...
    const [internalCheckedList, setInternalCheckedList] = useState<number[]>(checkedList);
    ...
    const allChecked = items.length > 0 && internalCheckedList.length === items.length;
    const isIndeterminate = internalCheckedList.length > 0 && internalCheckedList.length < items.length;
                ...
                {showSearch && (
                    <form className="w-full">
                        <input
                            ...
                            placeholder="بحث"
                            className="focus:outline-none block min-w-[300px] p-4 ps-10 text-sm rounded-lg bg-[#ADAAAA11]"
                        />
                    </form>
                )}
                ...
                <table className="w-full text-sm text-left rtl:text-right text-gray-500">
                    <thead className="text-[#38433B8F] uppercase">
                        <tr className="bg-white border-b border-gray-100">
                            ...
                            {expandable && !expandableColumn && <th className="px-4 py-5"></th>}

                            {headItems.map((item, index) => (
                                <th key={index} className="px-4 py-5 font-bold">
                                    {item.text}
                                </th>
                            ))}
                        </tr>
                    ...
                    <tbody>
                        {items.map((item, index) => (
                            <React.Fragment key={item.id}>
                                        ...
                                        <td key={col.name} className="px-4 py-5">
                                            {expandableColumn === col.name ? (
                                                <div onClick={() => toggleRow(index)}>
                                                    {item[col.name] ?? "-"} {expandedRow === index ? "▲" : "▼"}
                                                </div>
                                            ) : (
                                                item[col.name] ?? "-"
                                            )}
                                ...
                                {expandedRow === index && (
                                    <tr>
                                        <td colSpan={headItems.length + (checkedList ? 1 : 0) + (expandable && !expandableColumn ? 1 : 0)}>
                                            {children}
                                        </td>
                                    </tr>
                                )}
