...
import {
  chartStatisticsHomeService,
  collectorsHomeService,
  paymentsHomeService,
...
export default function Home() {
  ...
  const [statistics, setStatistics] = useState([
    ...
    {
      title: " عدد الزيارات ",
      subtitle: "الغير مكتملة",
      slug: "not_collected_visited",
    },
    {
      title: "عدد المستخدمين ",
      subtitle: "المشتركين",
      slug: "no_of_subscriptions",
    },
    {
      title: "عدد  المستخدمين ",
      subtitle: "الغير مشتركين",
      slug: "no_of_none_subscriptions",
    },
  ]);
  ...
  const [selectedPackageDate, setSelectedPackageDate] = useState<Date | null>(
    null
  );
  const [packageYear, setPackageYear] = useState("");

  const paymentList = ["rejected", "accepted", "pending"];
  ...
  const fetchChartStatistics = ({
    category_month = selectedMonth,
    category_year = year,
    package_month = selectedPackageMonth,
    ...
    const hasCategoryMonth = category_month
      ? "&category_month=" + category_month
      : "";
    const hasCategoryYear = category_year
      ? "&category_year=" + category_year
      : "";
    const hasPackageMonth = package_month
    ...
    chartStatisticsHomeService(query).then((response) => {
      ...
      const package_no_subscription = response.data.statsPackage.map(
        (item, index) => {
          return item.no_of_subscriptions;
        }
      );
          ...
          <div className="bg-[#00000009] p-3 rounded-3xl">
            <div className="grid xl:grid-cols-4 lg:grid-cols-2  grid-cols-1   gap-5">
              {statistics.map((item, index) => (
                <div
                  key={index}
                  className="flex justify-between rounded-3xl bg-background  p-5 w-full"
                >
                  ...
                  <div className=" text-[#38433B] text-2xl font-bold">
                    {userStatistics && (
                      <span>
                        {item.slug === "completed_visited"
                          ? `${userStatistics.total_visits_count} / ${
                              userStatistics[item.slug]
                            }`
                          : userStatistics[item.slug]}
                      </span>
                    )}
            ...
            <div className="grid grid-cols-2 gap-5">
              ...
              <div className="lg:col-span-1 col-span-2 rounded-2xl bg-background  p-7 w-full ">
                ...
                <div className="aspect-[3/3] overflow-auto">
                  ...
                  {collectors &&
                  collectors.notes_visit &&
                  collectors.notes_visit.length > 0 ? (
                    collectors.notes_visit.map((item, index) => (
                    ...
                    <div className="flex flex-col h-full justify-center items-center  py-10 text-center text-[#ADAAAA]">
                      <span className="mdi mdi-account-off text-4xl mb-3 text-[#009414]"></span>
                      <p className="text-base font-medium">
                        لا يوجد أي ملاحظات حالياً
                      </p>
          ...
          <div className="grid xl:grid-cols-2 grid-cols-1 gap-5">
              ...
              <div className="rounded-2xl bg-background  p-5 w-full relative ">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative">
                    <DatePicker
                      className=" bg-[#0094140A] text-[#009414] py-3 px-4 rounded-lg min-w-32"
            ...
            <div className="bg-[#00000009] p-3 rounded-3xl">
              <div className="rounded-2xl bg-background  p-5 w-full ">
                ...
                <ReactApexChart
                  options={optionsBarPackage}
                  series={seriesBarPackage}
                  type="bar"
                  height={350}
                />
              </div>
              ...
              <div className="rounded-2xl bg-background  p-5 w-full h-[500px] overflow-y-auto ">
                <div className="flex justify-between mb-3">
                  ...
                  <div>
                    <button className="border-none outline-none ">
                      <Link className="text-[#009414]" href="/payments">
                        عرض المزيد
                      </Link>
                ...
                {payment &&
                  payment.map((item, index) => (
                    <div
                      key={index}
                      className="bg-[#00000009] rounded-3xl px-2 py-1 mb-4"
                    >
                      <div key={index} className="flex justify-between py-5">
                        <div className="flex  gap-4">
                          ...
                          <div className="">
                            <p className="mb-1 font-bold text-[#38433B] text-lg">
                              {item.user_name}
                            </p>
                            <p className="text-[#009414] font-bold mb-1">
                              {item.total_price}
                            </p>
                ...
                <div className="flex justify-between mb-3">
                  <div>
                    <h4 className="font-bold mb-5 text-lg">
                      المستخدمين “ اعادة التدوير “
                    </h4>
                  </div>
                  <div>
                    <button className="border-none outline-none ">
                      <Link
                        className="text-[#009414]"
                        href="/users?is_request_recycle=1"
                      >
                        عرض المزيد
                      </Link>
                      <span className="mdi mdi-chevron-left text-[#009414] ms-5 text-xl"></span>
                        ...
                        {headerArr.map((item, index) => (
                          <th key={index} scope="col" className="px-4 py-5 ">
                            <div className="flex gap-2 text-nowrap font-bold">
                              <div className="flex flex-col cursor-pointer space-y-0 justify-center items-center leading-none "></div>
                      ...
                      {userListRecycle.map((item, index) => (
                        ...
                        >
                          ...
                          <td className="py-2 px-4">
                            {item.all_recycle_weights}
                          </td>
                          <td className="py-2 px-4">
                            {item.deserved_money_by_recycle}
                          </td>
                          <td className="py-2 px-4">
                            <button
                              onClick={() => handleSelectedUser(item)}
                              className="bg-blue-100 p-1 px-2 rounded-lg"
                            >
